...
namespace rjk {
    ...
    template<typename E, bool Enumerable = std::meta::is_enumerable_type(^^E)> requires std::is_enum_v<E>
...
inline namespace tags {
    ...
    template <std::meta::operators Op, any_tag... Tags>
    ...
    consteval bool check_unary_op() {
        if constexpr (Op == op_plus)      return requires(Operand o) { { +o } -> std::same_as<Ret>; };
        if constexpr (Op == op_minus)     return requires(Operand o) { { -o } -> std::same_as<Ret>; };
        return false;
    }

    template <std::meta::operators Op, typename Lhs, typename Rhs, typename Ret>
    consteval bool check_op() {
        if constexpr (Op == op_plus)      return requires(Lhs l, Rhs r) { { l + r } -> std::same_as<Ret>; };
        if constexpr (Op == op_minus)     return requires(Lhs l, Rhs r) { { l - r } -> std::same_as<Ret>; };
        return false;
    }
