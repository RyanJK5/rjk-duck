...
namespace rjk {
    ...
    using fn_arg_t = typename std::tuple_element<Index, typename decompose_fn_trait<Func>::args>::type;

    template <typename Func>
    constexpr auto fn_arg_count_v = std::tuple_size_v<typename decompose_fn_trait<Func>::args>;
}