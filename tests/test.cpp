...
TEST(AnyConstruct, InPlaceTypeInitializerList) {
    ...
    struct FromIL {
        int sum = 0;
        FromIL(std::initializer_list<int> il) { for (int v : il) sum += v; }
        void test()      {}
...
TEST(AnyMove, MoveOnlyType) {
    rjk::any<rjk::has_fn<"test", void()>, rjk::has_fn<"other", int(char)>> x{MoveOnly{7}};
...
TEST(AnyQualifiers, ConstMethod) {
    rjk::any<rjk::has_fn<"get", int() const>> x{WithConst{}};
    EXPECT_EQ(x->get(), 42);
}

TEST(AnyQualifiers, ConstMethodOnConstAny) {
    const rjk::any<rjk::has_fn<"get", int() const>> x{WithConst{}};
    EXPECT_EQ(x->get(), 42);
}
...
TEST(AnyQualifiers, LvalueRefMethod) {
    rjk::any<rjk::has_fn<"lvalue_fn", int() &>> x{WithLvalueRef{}};
    EXPECT_EQ(x->lvalue_fn(), 1);
}

TEST(AnyQualifiers, RvalueRefMethod) {
    rjk::any<rjk::has_fn<"rvalue_fn", int() &&>> x{WithRvalueRef{}};
...
using Sizeable = rjk::any<
    rjk::has_fn<"size",  std::size_t() const>,
    rjk::has_fn<"clear", void()>,
    rjk::has_fn<"empty", bool() const>
>;
static_assert(rjk::remove_noexcept(type_of(^^std::vector<int>::size)) == rjk::remove_noexcept(^^std::size_t() const));
